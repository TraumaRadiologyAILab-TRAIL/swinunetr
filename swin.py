...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        # Add segmentation heads for each decoder level
        self.out_1536 = UnetOutBlock(spatial_dims=3, in_channels=1536, out_channels=out_channels)
        self.out_768 = UnetOutBlock(spatial_dims=3, in_channels=768, out_channels=out_channels)
        self.out_384 = UnetOutBlock(spatial_dims=3, in_channels=384, out_channels=out_channels)
        self.out_192 = UnetOutBlock(spatial_dims=3, in_channels=192, out_channels=out_channels)
        self.out_96 = UnetOutBlock(spatial_dims=3, in_channels=96, out_channels=out_channels)

        self.out = UnetOutBlock(spatial_dims=spatial_dims, in_channels=feature_size, out_channels=out_channels)
        #add a linear layer for multi task learning
        self.out2 = nn.Linear(feature_size, 2)
    ...
    def forward(self, x_in, is_training=True):
        ...
        feature_maps = []
        feature_maps.append(self.out_1536(dec4))
        # print(self.out_1536(dec4).shape) #size = bs, 1536, 3,3,3
        dec3 = self.decoder5(dec4, hidden_states_out[3])
        feature_maps.append(self.out_768(dec3))
        # print(self.out_768(dec3).shape) #size = bs, 768, 6,6,6
        dec2 = self.decoder4(dec3, enc3)
        feature_maps.append(self.out_384(dec2))
        # print(self.out_384(dec2).shape) #size = bs, 384, 12,12,12
        dec1 = self.decoder3(dec2, enc2)
        feature_maps.append(self.out_192(dec1))
        # print(self.out_192(dec1).shape) #size = bs, 192, 24,24,24
        dec0 = self.decoder2(dec1, enc1)
        feature_maps.append(self.out_96(dec0))
        # print(self.out_96(dec0).shape) #size = bs, 96, 48,48,48
        out = self.decoder1(dec0, enc0)
        # print(out.shape) #size = bs, feature_size, h, w, d
        segmentation = self.out(out)
        # Global average pooling over spatial dimensions
        batch_size = out.shape[0]
        pooled = out.mean(dim=[2,3,4])  # Average over H,W,D dimensions
        logits = self.out2(pooled)  # Shape: [batch_size, 2]
        if is_training:
            return segmentation, logits, feature_maps
        else:
...
# #try out the model
# model = SwinUNETR(img_size=(96, 96, 128), in_channels=1, out_channels=3, feature_size=96)
# # print(model)
# #move model to gpu:7
...
# #move input tensor to gpu:7
# input_tensor = input_tensor.to('cuda:7')
